...
import Vue from 'vue'
import { Interest } from '@/client/types/api'
  ...
  methods: {
    ...
    confirmDelete (): void {
      this.$modal.show({
        type: 'danger',
        title: 'Please Confirm',
        body: 'Are you sure you want to delete this interest?',
        primary: {
          label: 'Confirm',
          theme: 'red',
          action: () => this.delete(),
    ...
    async save (): Promise<void> {
      ...
      try {
        await this.$axios.put('interests/save/' + this.interest.id, data)
    ...
    async delete (): Promise<void> {
      try {
        await this.$axios.delete('interests/delete/' + this.interest.id)
        this.$toast.success('Deleted!')
        this.$emit('get', true)
      } catch (e) {
        this.$toast.danger('Failed to delete interest!')
      }
    },
  },
})
